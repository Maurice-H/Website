...
description: "Optimize Vue 3 + Vite application performance including Core Web Vitals, bundle size, and CSS-variable-driven theme transitions."
---

# Web Performance Optimization (Vue 3 + Vite)

Tailored performance guidelines for the Premium Lamp Portfolio.

## Core Focus Areas

1.  **Vite Build Optimization:** Efficient code-splitting and asset compression.
2.  **Vue Runtime Performance:** Optimized reactivity and component updates.
3.  **Theme Transition Performance:** Ensuring smooth CSS variable transitions (neon glows/blueprint).
...
### 1. Code Splitting (Vue 3)
Instead of loading everything at once, use async components:
...
Ensure `vite.config.ts` is configured for optimal chunking.

### 3. CSS Variable Performance
Theme switching can be heavy if too many elements repaint.
- **DO:** Transition only `opacity` and `colors`.
- **DON'T:** Transition `box-shadow` or `filter` on every mouse move.

## Performance Checklist (MANDATORY)

- [ ] Convert all portfolio images to WebP/AVIF.
- [ ] Use `shallowRef` for large datasets in `src/data/`.
- [ ] Verify `var(--theme-transition-duration)` is consistent.
