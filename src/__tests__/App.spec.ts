...
describe('App.vue', () => {
  ...
  it('ignores ESC keypress in NAV phase', async () => {
    ...
    (lightingStore.setPhase as ReturnType<typeof vi.fn>).mockClear();
  ...
  it('renders css mask correctly', async () => {
    ...
    expect((contentStage.element as HTMLElement).style.getPropertyValue('--reveal-mask')).toContain(
      'radial-gradient'
    );
    ...
    lightingStore.phase = LightingPhase.CONTENT;
    await wrapper.vm.$nextTick();
    expect((contentStage.element as HTMLElement).style.getPropertyValue('--reveal-mask')).toBe('');

    // Lighting off -> empty style

    themeStore.lightingEnabled = false;
    await wrapper.vm.$nextTick();
    expect((contentStage.element as HTMLElement).style.length).toBe(0);
  });
