<template>
  ...
  <BentoCard id="about-discovery" data-testid="discovery-card" class="md:col-span-2 md:row-span-1" with-window :title="$t('skills.discoveryPath')" :is-low-end="performance.isLowEnd">
    <div class="p-4 md:p-6 flex flex-col">
      <p class="whitespace-pre-wrap text-finished-text/50 text-sm md:text-base leading-relaxed max-w-2xl transition-colors duration-[var(--theme-transition-duration)]">
        {{ $t('skills.bio') }}
      </p>
    </div>
  </BentoCard>
  ...
  <BentoCard id="skills-stack" class="md:col-span-2 md:row-span-1" with-window :title="$t('skills.stackTitle')" :is-low-end="performance.isLowEnd">
    <div ref="skillsContainerRef" class="p-4 md:p-6 flex flex-col h-full">
      <div 
        ...
        :style="[
          wrapStyle,
          (!showAll && !allFit) ? { maskImage: 'linear-gradient(to bottom, black 80%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 80%, transparent 100%)' } : {}
        ]"
      >
        <div v-for="category in categories" :key="category.name" class="flex flex-col gap-2 pb-2 border-b border-finished-text/5 last:border-0 last:pb-0">
          ...
          >
            ...
            <span class="text-finished-accent/50 group-hover:text-finished-accent transition-transform duration-300" :class="{ 'rotate-180': expandedCategories.has(category.name) }">
