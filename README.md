...
- **Model Comparison — Claude Opus vs. Gemini:** A notable observation was the difference in how models handled the custom skill files and `opsx` workflows. **Claude Opus 4.6 demonstrated significantly higher adherence to the project's skill MDs and workflow definitions**, reading them proactively and following their instructions with minimal deviation. Its chain-of-thought reasoning also felt more structured and logically coherent, often catching edge cases before they became bugs. Gemini 3.1 Pro, by contrast, required significantly more micro-prompting to stay on track and frequently drifted from established patterns as context grew. Other models (e.g., Codex, Kimi) were not tested, as the current Antigravity ecosystem and its agentic infrastructure are purpose-built for the Google and Anthropic model suites, leveraging a combination of Free Tier access and a 4-month Google AI Pro trial to sustain the high token demands of an autonomous workflow.
